...
<?php } ?>
          ...
          <div id="wallet-assets">
          </div>
          <template id="wallet-asset">
            <div class="panel panel-default" style="margin-bottom:2px; border-radius:0 30px 0 0;">
              <div class="panel-body" style="margin-right:-100px;">
                <div style="color:#777777; padding:10px 120px 0px 0px; font-size:18px; font-family:Arial; margin-top:-20px;">
                  <span class="temp-title"></span>
                  <div style="font-size:12px;">
                    <span>Published:</span> <span class="temp-date" style="color:#F39C12;"></span><br>
                    <span style="padding-right:10px;"><i class="fas fa-heart"></i> <span class="temp-hearts"></span></span>
                    <span style="padding-right:10px;"><i class="fas fa-headphones"></i> <span class="temp-plays"></span></span>
                    <span style="padding-right:10px;"><i class="fas fa-share"></i> <span class="temp-shared"></span></span>
                    <span><strong>NFT</strong> <i class="far fa-check-circle" style="color:#27AE60;"></i></span>
                  </div>
                </div>
              </div>
              <span align="right"><img src="" class="temp-image" style="height:100px; width:100px; border-radius:0 30px 0 30px;"></span>
            </div>
          </template>
        </div>
...
?>
  ...
  const assetTemplate = $('#wallet-asset').prop('content')
  ...
  async function getWalletAssets (pkh, fa2 = 'KT1MR8e46WJBq4RcFSogiDbSg3ceDRi81hpE', network = 'mainnet') {
    const tokens = await getTokens(pkh, fa2, network)
    const balances = tokens.balances
    for (let i = 0; i < balances.length; i++) {
      const elem = $(assetTemplate).clone(true, true)
      const id = balances[i].name
      const metadata = await getMetadata(id)
      $(elem).find('.temp-title').text(metadata.title.join('; ')).removeClass('temp-title')
      $(elem).find('.temp-date').text(metadata.published).removeClass('temp-date')
      $(elem).find('.temp-hearts').text(metadata.isLoveIt).removeClass('temp-hearts')
      $(elem).find('.temp-plays').text(metadata.playedCount).removeClass('temp-plays')
      $(elem).find('.temp-shared').text(metadata.isDedicate).removeClass('temp-shared')
      $(elem).find('.temp-image').attr('src', metadata.artwork).removeClass('temp-image')
      $('#wallet-assets').append(elem)
    }
  }

  async function getTokens (address, contract, network = 'mainnet') {
